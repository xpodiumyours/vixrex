...
- **Değişmez kural — asistan tek sözlükten konuşur, her yüzeyde:** Vixrex Asistan'ın kullanıcıya söylediği hiçbir cümle bileşenin/widget'ın içine elle yazılmaz. Tek kaynak `shared/vixrex_mesajlar.json` (16 niyet, 99 mesaj); Flutter onu `lib/config/vixrex_mesajlar.g.dart` üzerinden, web `public_web/src/lib/vixrexMesajlari.ts` üzerinden okur. Yeni bir cümle gerekiyorsa **önce kataloğa eklenir**, sonra `dart run tool/mesaj_semasi_uret.dart` ile Flutter tarafı tazelenir. Landing asistanı için bu kuralı `public_web/tests/landing-asistan-tek-kaynak.test.ts` kilitliyor — bileşene elle yazılan cümle testi kırmızıya düşürür.

  **Tarihçe ve kapsam (2026-08-26 → 27):** 26 Ağustos'ta ana sayfadaki sohbet sabit metinli bir maketti ve kural "motor bağlanmaz" diyordu; gerekçe, o gün ölçülen **dokuz ayrı "Vixrex Asistan" parçasına** onuncusunun eklenmesini önlemekti. 27 Ağustos'ta Casper bunu bilerek değiştirdi: maket, kullanıcıya "2 dakikada beraber hazırlayalım mı?" deyip cevap alamadığı için tam ikna anında tutulmayan bir sözdü. Sohbet gerçek hâle getirildi (`LandingAsistanSohbeti.tsx`), ama **onuncu beyin doğmadı** — akış `shared/` katalogundan okuyan ince bir çizici oldu.

  **Landing asistanının sınırı:** DEMO kipindedir, **veritabanına hiçbir şey yazmaz.** Ziyaretçi giriş yapmamıştır; orada kayıt oluşturmak çöp veri üretir. Cevaplar tarayıcı oturumunda tutulur (`vixrex_asistan_taslak`) ve kayıttan sonra vitrin kurulumuna taşınır. Yasal onay ve yayınlama adımları landing akışında **yoktur** — ikisi de hesap gerektirir. Bu sınırı kaldırmak kullanıcının açık isteğini gerektirir.

- **Değişmez kural — `sablonlar/hedef-vitrin.html` hedeftir, kopyalanacak dosya değildir:** Bu dosya üretim kodu değildir; vitrinin nasıl görünmesi gerektiğinin resmidir. Görünüş oradan alınır, **içerik alınmaz** — dosya teknik servis içeriğiyle doludur (FixTech, ekran, batarya). Yeni kategori bu dosya kopyalanarak yapılmaz; kategori farkları yalnız `public_web/src/lib/vitrinProfile.ts` dosyasına satır eklenerek tanımlanır, vitrini tek bir React bileşeni çizer. Bu kural 2026-08-05'te kondu.
...
- **Owner yetkilendirmesi tek kanaldan geçmiyor, bunu varsayarak kod yazma (2026-08-30 denetimi):** Next.js'te sahiplik `auth.uid()`'e değil asıl olarak `vixrex_owner_session` HMAC-imzalı HttpOnly çerezine ve `edit_token`'a dayanır (`public_web/src/lib/ownerSession.ts`, RPC `create_owner_session`). `create_owner_session`, `auth.uid() = stores.user_id` VEYA `edit_token` eşleşmesini eşit derecede geçerli sayar — yani sahip Supabase Auth ile hiç giriş yapmadan da (yalnız token'ı elinde tutarak) tam CRUD yetkisi alabilir. Bu kasıtlı bir tasarımdır (önizleme/handoff/rent-demo akışları için gerekli) ve **kaldırılacak bir hata değildir**; ama "sahiplik = auth.uid()" varsayımıyla yeni yetkilendirme kodu yazılırken bu ikinci kanal hesaba katılmalıdır — aksi halde ya gerçek bir güvenlik açığı (edit_token'ı kontrol etmeyi unutmak) ya da yanlış "kırık" teşhisi (auth.uid() eşleşmiyor diye reddetmek) ortaya çıkar.
...
- **Ürün kategorisi için ilişkisel `product_categories` tablosu tek gerçek kaynaktır, `stores.product_categories` JSONB kolonu değil (2026-08-30 denetimi):** Next.js (`/api/product-categories`, RPC `upsert_store_category`) doğru şekilde ilişkisel tabloya yazıyor; Flutter'ın kategori ekranı (`product_category_management_screen.dart`) hâlâ yerel bellekte sahte ID üretip yalnız legacy `stores.product_categories` JSONB'sini güncelliyor, ilişkisel tabloya hiç yazmıyor — Flutter'da oluşturulan kategoriler Next.js'te görünmez ve ürün gerçek kategoriye bağlanamaz (kategorisiz kalır). Bu alanda çalışan bir sonraki değişiklik önce `dart analyze`/`grep -rn "upsert_store_category" lib/` ile zincirin hâlâ kopuk olduğunu doğrulamalı; düzeltme Flutter tarafını ilişkisel tabloya/RPC'ye bağlamak olmalı, Next.js'in doğru çalışan yolunu değiştirmek değil. Detay: `docs/research/vixrex-web-mobil-tamlik-denetimi-2026-08-30.md`.
