...
- **Değişmez kural — landing'deki asistan sohbeti bir makettir, motor bağlanmaz:** Ana sayfadaki telefon mockup'ının içinde açılan Vixrex Asistan sohbeti (`public_web/src/components/landing/PhoneMockup.tsx` → `AsistanSohbetIcerigi`) bir **reklam görselidir**: sabit metin, kullanıcı yazamaz, hiçbir sunucuya bağlı değildir ve bağlanmayacaktır. Amacı ürünü anlatmak, vitrin kurmak değil. **Web'de gerçek asistan yalnızca sahip panelindedir** (`/v/:slug` → `OwnerAssistantPanel`); landing maketi ona alternatif bir giriş kapısı değildir. Maskot düğmesi de yalnız bu maketi açıp kapatır. Bu kural 2026-08-26'da kondu: o gün ölçüldüğünde iki yüzeyde toplam **dokuz ayrı "Vixrex Asistan" parçası** vardı (Flutter'da 6, web'de 3) ve landing maketini "çalışmıyor" sanıp gerçek motora bağlamak dördüncü bir web asistanı doğuracaktı. Landing sohbetini işlevsel hâle getirmek isteyen her değişiklik kullanıcının açık isteğini gerektirir.
