...
import 'package:vixrex/services/invoice_catalog/invoice_draft_trace_resolver.dart';
import 'package:vixrex/services/invoice_catalog/invoice_ocr_draft_adapter.dart';
...
class OcrService {
  ...
  final InvoiceOcrDraftAdapter _invoiceDraftAdapter;
  final InvoiceDraftTraceResolver _invoiceTraceResolver;
  ...
  const OcrService({
    ...
    InvoiceOcrDraftAdapter? invoiceDraftAdapter,
    InvoiceDraftTraceResolver? invoiceTraceResolver,
       ...
       _invoiceDraftAdapter =
           invoiceDraftAdapter ?? const InvoiceOcrDraftAdapter(),
       _invoiceTraceResolver =
           invoiceTraceResolver ?? const InvoiceDraftTraceResolver();
  ...
  Future<Result<OcrCatalogResult>> _analyzeInvoice(Uint8List imageBytes) async {
    ...
    return Result.success(
      OcrCatalogResult(
        ...
        invoiceDrafts: best.invoiceDrafts,
  ...
  Future<_InvoiceAttempt> _invoiceAttempt(Uint8List imageBytes) async {
    ...
    final rawDrafts = _invoiceDraftAdapter.fromProducts(products);
    final invoiceDrafts = await _invoiceTraceResolver.resolve(rawDrafts);
