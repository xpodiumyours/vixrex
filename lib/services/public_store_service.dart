...
class PublicStoreService {
  ...
  Future<Result<Map<String, dynamic>?>> fetchPublishedStoreBySlug(String slug) async {
    try {
  ...
  Future<Result<Map<String, dynamic>?>> fetchPublishedStoreProducts(String slug) async {
    try {
      final res = await _resolveClient
          .from('stores')
