...
class VitrinEditorScreen extends StatefulWidget {
  ...
  const VitrinEditorScreen({super.key, this.initialStoreName});
...
class _VitrinEditorScreenState extends State<VitrinEditorScreen> {
  ...
  Future<void> _loadSavedData() async {
    try {
      ...
      final token = prefs.getString(LocalStorageKeys.vitrinEditToken);
  ...
  Future<void> _publishStore() async {
    ...
    final validationMessage = const StorePublishValidator().validateVitrin(
      _data,
    );
  ...
  Future<String> _loadOrCreateEditToken() async {
    final prefs = await SharedPreferences.getInstance();
    final String key = LocalStorageKeys.vitrinEditToken;
            ...
            children: [
              ...
              _buildScoreTargetAnchor(
                ...
                _buildTextField(
                  ...
                  hintText: 'Vitrininizi kısaca anlatın',
                ),
              ),
              const SizedBox(height: 16),
              _buildTextField(
                ...
                hintText: 'Örn: Bugün yeni ürünler geldi!',
  ...
  /// Vitrin slug'\u0131n\u0131 \u00fcretir.
  /// Delegasyonu [StorePublishPayloadBuilder.generateSlug]'a b\u0131rak\u0131r.
  String _generateStoreSlug(String name) =>
      const StorePublishPayloadBuilder().generateSlug(name);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color.fromRGBO(255, 255, 255, 0.94),
        borderRadius: BorderRadius.circular(999),
        border: Border.all(color: _VitrinEditorScreenState.cardBorder),
      ),
      child: Text(
        label,
        style: TextStyle(
          color: _VitrinEditorScreenState.softText.withValues(alpha: 0.9),
          fontSize: 10,
          fontWeight: FontWeight.w800,
        ),
      ),
