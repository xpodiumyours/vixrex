...
/// doğrudan test ediyor. Paylaşılan bir test/support dosyasına çıkarmak
/// makul bir sonraki-faz temizliği ama şimdilik küçük tekrar kabul edilebilir.
...
void main() {
  ...
  group('syncCatalog', () {
    ...
    test('virgüllü/noktalı fiyat metnini doğru ayrıştırır', () async {
      await service.syncCatalog(
        storeId: storeId,
        editToken: editToken,
        products: [Product(id: 'local-1', name: 'A', price: '1.234,56 TL')],
      );
  ...
  group('addProduct', () {
    test('başarılıysa ürünü sunucu id/slug ile yerinde günceller', () async {
      final product = Product(id: 'local-1', name: 'Kazak');
