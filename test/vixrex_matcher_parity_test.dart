  ...
  group('Vixrex matcher shared parity fixtures — Flutter', () {
    for (final fixture in fixtures) {
      test(fixture['id'] as String, () {
        ...
        final expectedFields = (fixture['expectedFieldKeys'] as List).cast<String>();
        final expectedClasses = (fixture['expectedMatchClasses'] as List).cast<String>();

        expect(matches.map((match) => match.alan.anahtar).toList(), expectedFields);
        expect(matches.map((match) => match.matchClass).toList(), expectedClasses);
      });
    }
  });
}
