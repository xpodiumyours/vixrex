...
void main() {
  group('BusinessCategoryConfig & SuggestedOffering Tests', () {
    test(
      'SuggestedOffering can hold durationMinutes and isBookable properties',
      () {
    ...
    test(
      'Kuaför category has correct rich suggested offerings with default booking options',
      () {
        final kuafor = BusinessCategoryConfig.categories.firstWhere(
          (c) => c.id == 'kuafor',
        );
        ...
        final hairCut = kuafor.suggestedOfferings.firstWhere(
          (o) => o.title == 'Saç Kesimi & Yıkama',
        );
        expect(hairCut.durationMinutes, 45);
        expect(hairCut.isBookable, true);

        final skinCare = kuafor.suggestedOfferings.firstWhere(
          (o) => o.title == 'Medikal Cilt Bakımı',
        );
    ...
    test('Teknik Servis category has correct bookable suggested offerings', () {
      final service = BusinessCategoryConfig.categories.firstWhere(
        (c) => c.id == 'teknik_servis',
      );

      expect(service.suggestedOfferings, isNotEmpty);
      final screenRep = service.suggestedOfferings.firstWhere(
        (o) => o.title == 'Telefon Ekran Değişimi',
      );
    ...
    test('fromCategoryLabel maps categories correctly', () {
      final categoryKuafor = BusinessCategoryConfig.fromCategoryLabel(
        'Güzellik Salonu',
      );
      expect(categoryKuafor.id, 'kuafor');

      final categoryRestoran = BusinessCategoryConfig.fromCategoryLabel(
        'Restoran',
      );
