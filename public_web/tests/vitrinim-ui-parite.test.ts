...
describe("Vitrinim görsel kabuk paritesi", () => {
  ...
  it("masaüstünde Flutter Wrap gibi 24px sütun aralığı kullanır ve kısa bölümü esnetmez", () => {
    expect(flutterForm).toContain("final isDesktop = screenWidth > 900");
    expect(flutterForm).toContain("(constraints.maxWidth - AppColors.spacing24) / 2");
    expect(flutterForm).toContain("spacing: AppColors.spacing24");
    expect(webEditor).toContain("min-[901px]:grid-cols-2");
    expect(appCss).toContain("column-gap: 24px");
    expect(appCss).toContain("align-items: start");
  });
