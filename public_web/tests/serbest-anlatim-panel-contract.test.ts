 ...
 */
describe("Serbest anlatım daveti — panel açılınca, tek-tek soru akışından önce çıkar", () => {
  const panel = oku("app/v/[slug]/OwnerAssistantPanel.tsx");

  it("davet yalnız zorunlu alan eksikken ve oturum başına bir kez çıkar", () => {
  ...
  it("davet mesajı iki hızlı cevap taşır: anlatayım / tek tek sor", () => {
    expect(panel).toContain('{ label: "Anlatayım", payload: "serbest_anlatimi_ac" }');
    expect(panel).toContain('{ label: "Tek tek sor", payload: "ilk_eksik_alana_git" }');
  });

  it("'tek tek sor' zaten var olan ilk_eksik_alana_git payload'ını kullanır — ayrı bir atlama mekanizması icat edilmedi", () => {
    expect(panel).toContain('if (payload === "serbest_anlatimi_ac")');
