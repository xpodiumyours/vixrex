...
describe("blog yayın anahtarı ve içerik sözleşmesi", () => {
  ...
  it("arşiv içerik yayinda true olsa bile dışarı çıkmaz", () => {
    const arsiv = {
      ...BLOG_YAZILARI[0],
      slug: "arsiv-test",
    ...
    expect(yaziDurumunuHesapla(arsiv, new Date("2026-09-10T00:00:00Z"))).toBe("arsiv");
  });
  ...
  it("her yazının zorunlu kalite alanları dolu", () => {
    ...
    for (const yazi of BLOG_YAZILARI) {
      ...
      expect(["harici_platform", "vixrex_urun", "genel"]).toContain(yazi.kontrolSinifi);
  ...
  it("ham HTML taşımaz; sınırlı Markdown başlıkları ayrıştırılır", () => {
    for (const yazi of BLOG_YAZILARI) {
      ...
      expect(icindekileriCikar(yazi.govde).every((x) => x.id.length > 0)).toBe(true);
    }
  });

  it("aynı veya işaretten oluşan başlıklar benzersiz ve boş olmayan id üretir", () => {
    const icindekiler = icindekileriCikar("## Aynı başlık\n\n## Aynı başlık\n\n### ???");
