...
export function AboutEditor({ slug, mevcut, onClose, inline = false }: Props) {
  ...
  const content = (
    <>
        <div className={`sticky top-0 z-10 flex items-center justify-between border-b px-5 py-4 ${inline ? "border-lp-border bg-lp-surface" : "border-white/10 bg-[#0B1120]"}`}>
          <div>
            <h2 className={`text-lg font-extrabold ${inline ? "text-lp-text" : "text-white"}`}>Hakkımızda</h2>
            <p className={`text-[11px] ${inline ? "text-lp-muted" : "text-white/40"}`}>Boş bırakırsan vitrinde bu bölüm gizlenir.</p>
          </div>
