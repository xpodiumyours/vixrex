...
export function CampaignEditor({ slug, mevcut, onClose, inline = false }: Props) {
  ...
  const content = (
    <>
        <div className={`sticky top-0 z-10 flex items-center justify-between border-b px-5 py-4 ${inline ? "border-lp-border bg-lp-surface" : "border-white/10 bg-[#0B1120]"}`}>
          <div>
            <h2 className={`text-lg font-extrabold ${inline ? "text-lp-text" : "text-white"}`}>Öne Çıkan Kampanya</h2>
            <p className={`text-[11px] ${inline ? "text-lp-muted" : "text-white/40"}`}>Tümünü boş bırakırsan kampanya bölümü gizlenir.</p>
          </div>
        ...
        <div className={`sticky bottom-0 border-t px-5 py-3 flex gap-3 ${inline ? "border-lp-border bg-lp-surface" : "border-white/10 bg-[#0B1120]"}`}>
          <button
            onClick={onClose}
            className={`flex-1 rounded-xl border py-2.5 text-xs font-bold transition ${inline ? "border-lp-border text-lp-muted hover:bg-lp-surface-soft" : "border-white/20 text-white/60 hover:bg-white/5"}`}
          >
            İptal
          </button>
          <button
