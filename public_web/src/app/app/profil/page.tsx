...
function YardimIkonu() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
      ...
      <path d="M9.8 9a2.4 2.4 0 1 1 3.8 1.9c-.9.6-1.6 1.1-1.6 2.3" />
      <path d="M12 17h.01" />
    </svg>
  );
}
...
export default function ProfilPage() {
  ...
  if (yukleniyor) {
    return (
      <main className="flex min-h-full items-center justify-center bg-lp-bg-editor px-4">
        <div className="owner-card flex items-center gap-3 px-5 py-4" role="status" aria-live="polite">
          <span className="h-3 w-3 animate-pulse rounded-full bg-lp-primary" aria-hidden="true" />
          <span className="text-sm text-lp-muted">Yükleniyor…</span>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-full bg-lp-bg-editor text-lp-text">
      <header className="flex h-14 items-center bg-lp-bg-editor px-6">
        <h1 className="text-xl font-black">Profil</h1>
      </header>

      <div className="w-full space-y-0 px-6 pb-8 pt-2">
        {mesaj ? (
          <p className="mb-4 rounded-2xl border border-lp-border bg-lp-surface px-4 py-3 text-sm text-lp-muted" role="status">
            {mesaj}
          </p>
        ) : null}
        ...
        <section className="rounded-2xl border border-lp-border bg-lp-surface p-4">
          <div className="flex items-center gap-3.5">
            <div className="flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-full bg-lp-surface-soft text-lp-primary">
              <KisiIkonu />
            </div>
            <div className="min-w-0">
              <p className="truncate text-[15px] font-black text-lp-text">{kullaniciEtiketi}</p>
        ...
        <button
          ...
          className="w-full rounded-2xl border border-lp-border bg-lp-surface p-4 text-left"
        >
          <p className="text-[12px] font-black text-lp-text">Vitrin Bağlantısı</p>
          <div className="mt-2 flex items-center gap-2 rounded-xl bg-lp-surface-soft px-3 py-2.5">
            ...
            <span className={`min-w-0 flex-1 truncate text-[11px] font-bold ${publicLink ? "text-lp-text" : "text-lp-muted"}`}>
              {publicLink
        ...
        <button
          type="button"
          onClick={qrAc}
          className="flex w-full items-center gap-3.5 rounded-2xl border border-lp-border bg-lp-surface p-4 text-left"
        >
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-lp-primary/10 text-lp-primary">
            <QrIkonu />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-black text-lp-text">Hızlı QR Kod Paylaşımı</p>
            <p className="mt-0.5 text-[11px] font-semibold text-lp-muted">Vitrin QR kodunuza hızlıca ulaşın.</p>
          </div>
        ...
        <Link href="/app/ayarlar" className="flex items-center gap-3.5 rounded-2xl border border-lp-border bg-lp-surface p-4 text-lp-muted hover:bg-lp-surface-soft">
          <AyarIkonu />
          <span className="flex-1 text-[12px] font-black text-lp-text">Uygulama Ayarları</span>
          <OkIkonu />
        </Link>
        <div className="h-2.5" />
        <Link href="/yardim" className="flex items-center gap-3.5 rounded-2xl border border-lp-border bg-lp-surface p-4 text-lp-muted hover:bg-lp-surface-soft">
          <YardimIkonu />
          <span className="flex-1 text-[12px] font-black text-lp-text">Kullanım Bilgisi &amp; Destek</span>
          <OkIkonu />
        </Link>
        <div className="h-2.5" />
        <Link href="/legal/privacy" className="flex items-center gap-3.5 rounded-2xl border border-lp-border bg-lp-surface p-4 text-lp-muted hover:bg-lp-surface-soft">
          <KalkanIkonu />
          <span className="flex-1 text-[12px] font-black text-lp-text">Gizlilik ve Güvenlik politikası</span>
          <OkIkonu />
        </Link>
      </div>

      {qrAcik && publicLink ? (
        <div className="fixed inset-0 z-[70] flex items-end bg-black/60" onClick={() => setQrAcik(false)}>
          <div
          ...
          >
            ...
            <h2 className="mt-4 text-center text-[22px] font-black text-lp-text">Vitrin QR Kodunuz</h2>
            ...
            />
            <p className="mx-auto mt-3 max-w-[520px] truncate text-center text-[11px] font-semibold text-lp-muted">
              {typeof window !== "undefined" ? `${window.location.origin}${publicLink}` : publicLink}
            </p>
          </div>
        </div>
