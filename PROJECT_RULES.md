...
- Sticky butonlar
- Keşfet → işletme kartı → Düzenle/Vitrinim çekirdek akışı
...
- Yeni kullanıcı ekranında uygun yükleme, boş, hata ve yeniden deneme durumlarını
  aynı görevde ele al; yalnız mutlu akışı yapıp tamamlandı deme.
...
- Her tarama için `*_BORCU.md`, `*_TESHIS.md`, `NOT_*.md` gibi yeni defterler
  açma. Kalıcı kural `PROJECT_RULES.md`/`AGENTS.md` içinde; onaylı iş planı
  ilgili mevcut alan belgesi veya GitHub işi içinde tutulur. Tamamlanan işin
  geçmişi Git commit/PR kayıtlarıdır.

### 3.2 AI kaynak ve token disiplini (zorunlu)

Bu bölüm öneri değildir; bütün AI ajanları için bağlayıcı çalışma sınırıdır.
"Kurallar okundu" demek tek başına yeterli değildir, aşağıdaki sınırlar fiilen
uygulanır:

- Her görev için en küçük kanıt bütçesini kullan: **bir hedefli tarama, bir küçük
  değişiklik, değişen kapsama uygun bir doğrulama turu**. Kod, yapılandırma veya
  ortam değişmeden aynı test/build/analiz komutunu tekrar çalıştırma.
- Her araç çağrısından önce "Bu çıktı kararı değiştirecek mi?" diye kontrol et.
  Cevap hayırsa çağrıyı yapma. Alakasız repo geneli tarama, tam log dökümü,
  ekran ekran izleme ve alışkanlık amaçlı test yasaktır.
- CI, deploy veya başka bir uzak iş başlatıldıktan sonra ajan model içinde
  beklemez, uyumaz ve periyodik sorgulama yapmaz. Run bağlantısını/kimliğini ve
  son bilinen durumu bildirip kullanıcıya döner. Kullanıcı yeniden sormadıkça
  veya sistem tarafından uyandırılmadıkça aynı işi tekrar kontrol etmez.
- Hata incelemesinde önce yalnız başarısız adımı ve ilk eyleme dönük hatanın
  çevresindeki en fazla **80 satırı** oku. Bu kesit yetersiz kalmadan bütün logu
  yükleme veya konuşmaya taşıma.
- İki veya daha fazla tam build, repo genelinde derin tarama, bütün CI logunu
  okuma ya da uzun tarayıcı otomasyonu gerekecekse başlamadan önce Furkan'a
  kısa maliyet/kapsam uyarısı ver ve açık onay al.
- Kota, kapasite veya kaynak sınırı uyarısı görülürse zorunlu olmayan bütün
  işlemleri durdur; mevcut commit/run/engel bilgisini kısa biçimde devret.
- Sonuç raporu kısa olur: elde edilen sonuç, geçen doğrulama ve gerçekten kalan
  iş. Süreç günlüğünü, araç anlatımını ve aynı bilgiyi tekrar tekrar yazma.
