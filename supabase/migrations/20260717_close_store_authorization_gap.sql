...
BEGIN
  ...
  SET
    ...
    product_categories = pg_catalog.coalesce(p_store->'product_categories', product_categories),
  ...
  WHERE slug = pg_catalog.btrim(p_slug)
    AND (
      (
        pg_catalog.length(pg_catalog.btrim(pg_catalog.coalesce(p_edit_token, ''))) >= 24
