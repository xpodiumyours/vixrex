-- Dilim G: Hakkımızda geniş alanları + galeri bölüm meta + puan/yol tarifi anahtarları.

ALTER TABLE public.stores
  ADD COLUMN IF NOT EXISTS about_kicker TEXT,
  ADD COLUMN IF NOT EXISTS about_title TEXT,
  ADD COLUMN IF NOT EXISTS about_image_url TEXT,
  ADD COLUMN IF NOT EXISTS about_image_caption TEXT,
  ADD COLUMN IF NOT EXISTS about_values JSONB NOT NULL DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS gallery_section_kicker TEXT,
  ADD COLUMN IF NOT EXISTS gallery_section_title TEXT,
  ADD COLUMN IF NOT EXISTS show_storefront_rating BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS show_directions_link BOOLEAN NOT NULL DEFAULT true;

COMMENT ON COLUMN public.stores.about_kicker IS
  'Hakkımızda üst etiketi (örn. Atmosfer’in hikâyesi).';
COMMENT ON COLUMN public.stores.about_title IS
  'Hakkımızda ana başlık.';
COMMENT ON COLUMN public.stores.about_image_url IS
  'Hakkımızda bölüm görseli.';
COMMENT ON COLUMN public.stores.about_image_caption IS
  'Hakkımızda görsel alt yazısı.';
COMMENT ON COLUMN public.stores.about_values IS
  'Hakkımızda değer kartları: [{id, title, description}, ...] en fazla 3.';
COMMENT ON COLUMN public.stores.gallery_section_kicker IS
  'Galeri bölüm üst etiketi.';
COMMENT ON COLUMN public.stores.gallery_section_title IS
  'Galeri bölüm başlığı.';
COMMENT ON COLUMN public.stores.show_storefront_rating IS
  'Vitrinde puan bandı gösterilsin mi (varsayılan kapalı).';
COMMENT ON COLUMN public.stores.show_directions_link IS
  'Vitrinde yol tarifi butonu gösterilsin mi (varsayılan açık).';
